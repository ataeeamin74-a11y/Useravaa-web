...
export const insightShareBrand = {
  logoAssetUrl: "/brand/useravaa-logo-horizontal.png"
} as const;
...
async function drawProviderAvatar(ctx: CanvasRenderingContext2D, data: InsightShareExportData, x: number, y: number, size: number) {
  ...
  if (data.provider.avatarUrl) {
    try {
      const avatar = await loadImage(resolveAssetUrl(data.provider.avatarUrl));
  ...
  ctx.arc(x + size / 2, y + size / 2, size / 2, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = "#071b4e";
  ctx.font = `700 92px ${insightShareFontFamily}`;
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  ctx.direction = "rtl";
  ctx.fillText(data.provider.initials, x + size / 2, y + size / 2 + 2);
  ctx.restore();
}
...
async function drawBrand(ctx: CanvasRenderingContext2D, data: InsightShareExportData) {
  try {
    ...
    const sourceX = Math.round(logo.width * 0.07);
    const sourceY = Math.round(logo.height * 0.35);
    const sourceWidth = Math.round(logo.width * 0.86);
    const sourceHeight = Math.round(logo.height * 0.34);
    const targetWidth = insightShareVisualSpec.logoDrawWidth;
    const targetHeight = Math.round(targetWidth * (sourceHeight / sourceWidth));

    ctx.drawImage(logo, sourceX, sourceY, sourceWidth, sourceHeight, 1294, 94, targetWidth, targetHeight);
    return;
  } catch {
    ctx.fillStyle = "#02a99b";
    ctx.font = `800 30px ${insightShareLatinFontFamily}`;
    ctx.textAlign = "right";
    ctx.direction = "ltr";
    ctx.fillText("Useravaa", 1488, 104);
  }
}
...
export async function renderInsightShareImage(data: InsightShareExportData) {
  ...
  gradient.addColorStop(0, "#e9f6ff");
  ...
  ctx.fillStyle = "rgba(249, 253, 255, 0.92)";
  ctx.fill();
  ctx.strokeStyle = "#d7e9fb";
  ctx.lineWidth = 2;
  ctx.stroke();
  ctx.restore();
