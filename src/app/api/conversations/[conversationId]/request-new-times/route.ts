...
import { newTimeRequestSchema } from "@/lib/backend/validation";
...
export async function POST(request: Request, context: ConversationRouteContext) {
  ...
  const payload = await parseJsonBody(request, newTimeRequestSchema);
  ...
  return serviceResultToResponse(await conversationService.requestNewTimes(auth.viewer, conversationId, payload));
}
