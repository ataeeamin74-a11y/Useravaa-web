...
import { adminCancellationCreditApprovalSchema } from "@/lib/backend/validation";
...
export async function POST(request: Request, context: AdminCancellationRouteContext) {
  ...
  const payload = await parseJsonBody(request, adminCancellationCreditApprovalSchema);
  ...
  return serviceResultToResponse(await adminCancellationService.approveCredit(auth.viewer, cancellationId, payload));
}
